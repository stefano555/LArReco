...
namespace lar_reco
{
StatusCode Info_CaloHitAlgorithm2::Run()
{
	...
	int track_counter=0;
	for(int j=0;j<drifttime_vector.size();j++)
	{
		if(id[j]==11 || id[j]==22)
		{
			shower_counter++;
	...
	float* drifttime_shower = new float [shower_counter]();
	float* zplane_shower = new float [shower_counter]();
	float* ex_shower = new float [shower_counter]();
	float* ey_shower = new float [shower_counter]();
	
	float* drifttime_track = new float [track_counter]();
	float* zplane_track = new float [track_counter]();
	float* ex_track = new float [track_counter]();
	float* ey_track = new float [track_counter]();

	int k_shower=0;
	int k_track=0;
	
	for(int k=0;k<drifttime_vector.size();k++)
	{
		if(id[k]==11 || id[k]==22)
		{
			drifttime_shower[k_shower]=drifttime[k];
			zplane_shower[k_shower]=zplane[k];
			ex_shower[k_shower]=ex[k];
			ey_shower[k_shower]=ey[k];
			k_shower++;
		}
		else
		{
			drifttime_track[k_track]=drifttime[k];
			zplane_track[k_track]=zplane[k];
			ex_track[k_track]=ex[k];
			ey_track[k_track]=ey[k];
			k_track++;
		}
	}
...
//////////////////
	...
	TGraphErrors *gr_shower = new TGraphErrors(shower_counter,drifttime_shower,zplane_shower,ex_shower,ey_shower);
	TGraphErrors *gr_track = new TGraphErrors(track_counter,drifttime_track,zplane_track,ex_track,ey_track);
	...
	//gr->Draw("P,same");
	gr_shower->SetLineColor(4);
	gr_shower->SetMarkerColor(4);
	gr_track->SetLineColor(2);
	gr_track->SetMarkerColor(2);
	gr_shower->Draw("P,same");
	gr_track->Draw("P,same");
...
StatusCode Info_CaloHitAlgorithm2::ReadSettings(const TiXmlHandle /*xmlHandle*/)
{
